...
import { LEVEL_LABEL, TYPE_LABEL, riskWindows, windowEndLabel, type HourScore } from './fog';
...
/**
 * Уведомляет о первом окне высокой вероятности в ближайшие 12 часов.
...
export async function maybeNotify(hours: HourScore[], coords: Coords, show: Show): Promise<boolean> {
  ...
  await show(`Туман: ${LEVEL_LABEL[w.peak.level].toLowerCase()} вероятность`, {
    body: `${from} до ${windowEndLabel(w)} · пик ${w.peak.score}% в ${w.peak.time.slice(11, 16)}${type}`,
    tag: 'fog-alert',
