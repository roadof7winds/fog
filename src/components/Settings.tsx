...
export function Settings({ coords, onCoords, notif, onEnableNotif, standalone, canInstall, onInstall }: Props) {
  ...
  return (
    <section className="panel settings" id="settings">
      ...
      {notif === 'granted' && (
        <p className="muted">
          Включены. Вечером (20:00–24:00) предупрежу про ближайшую ночь, утром в 7 повторю, если риск ещё впереди.
          Порог — {Math.round(ALERT_P * 100)}%.
        </p>
      )}
      {notif === 'default' && (
        <>
          <p className="muted">
            Предупрежу вечером (20:00–24:00) про ближайшую ночь и повторю утром в 7, если риск ещё впереди. Порог —
            {' '}{Math.round(ALERT_P * 100)}%, это примерно в {Math.round(ALERT_P / MODEL.baseRate)} раз выше обычного. Ночью будить не буду.
          </p>
      ...
      <p className="fine">
        Проверено на наблюдениях 2023–2026, которых модель не видела: при этом пороге выходит около 137 уведомлений
        в год, с туманом оказывается каждое шестое, а поймать удаётся 61% туманов. Туман плохо прогнозируется в
